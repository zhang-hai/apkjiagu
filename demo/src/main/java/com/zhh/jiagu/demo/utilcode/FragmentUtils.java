...
public final class FragmentUtils {
     ...
     * @param out
     */
    public static void remove(@NonNull final Fragment remove, int in, int out) {
        FragmentManager fm = remove.getFragmentManager();
        if (fm == null) {
            remove(remove);
        } else {
            FragmentTransaction ft = fm.beginTransaction();
            addAnim(ft, in, out, 0, 0);
            operate(TYPE_REMOVE_FRAGMENT, fm, ft, null, remove);
        }
    }
